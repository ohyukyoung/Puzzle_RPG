...
#### :zap:시작 배경 
평소 게임에 관심이 많았기 때문에 게임 관련 프로젝트를 만들고 싶었다. 마침 공모전이 있었고 주제는 자유주제였다. 같은 학교 학우들과 프로젝트를 만들기로 하였고 모두 게임에 관심이 있었기 때문에 게임 프로젝트를 하게 되었다. 팀원들 모두 게임제작을 해본 적이 없었기 때문에 어떤 툴로 만들어야할지 어떤 게임 개발 엔진을 사용해야할지도 몰랐지만 결국 그나마 c++에 익숙했던 팀원들과 나는 범용성이 가장 높은 유니티를 선택하였다. 시간은 2달 조금 덜 되게 있었고 모두 처음이었기 때문에 여러가지 소스를 클론코딩하면서 실력을 쌓았다.
